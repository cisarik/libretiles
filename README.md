...
Activation is deliberately manual: a credential plus probe PASS plus explicit Django Admin activation of the exact row through the reviewed fallback-order workflow (ordinary Admin saves cannot toggle `is_active` or `sort_order`). Then complete one live game. The minimum live MVP claim is one provider PASS and one completed game. Do not claim that all five providers work until every exact pair has a current live PASS. Rollback is Admin deactivation of the affected row; never delete the row or game history.

Django Admin also records bounded fake-by-default capability observations on catalog rows. Live admin probes stay gated by `PROVIDER_PROBE_LIVE=1` and do not run during ordinary tests.
