...
export function ReplayMoveDetails({ move, players, previousScores }: { move: AdminReplayPly | null; players: AdminReplayPlayer[]; previousScores?: [number | null, number | null] }) {
  ...
  return <section className={`${styles.panel} p-5`}><div className="text-xs font-black uppercase tracking-[.18em] text-amber-400">Stored sequence {move.seq}</div><h2 className="mt-2 text-xl font-black text-amber-100">{player}: {move.kind.replace("_", " ")}</h2><p className="mt-2 text-stone-300">Points gained: <strong>{move.points}</strong></p>{move.kind === "exchange" ? <p className="mt-2 text-stone-300">Exchanged {move.tiles_exchanged} tiles{move.exchanged_tiles ? `: ${move.exchanged_tiles.join(" ")}` : " (letters unavailable)"}</p> : null}{move.words_formed.length ? <div className="mt-4 space-y-3">{move.words_formed.map((word, index) => <article key={`${word.word}-${index}`} className="rounded-xl border border-amber-300/15 bg-black/25 p-3"><div className="font-black text-stone-100">{word.word} · {word.score} points</div><div className="mt-1 text-sm text-stone-400">Word multiplier: {word.multiplier ?? "unavailable"}</div><div className="text-sm text-stone-400">Coordinates: {word.coords?.map((cell) => `${cell.row + 1},${cell.col + 1}`).join(" · ") || "unavailable"}</div><div className="mt-1 text-xs text-stone-500">Stored score shown; detailed per-letter calculation is unavailable when replay inputs do not reconcile.</div></article>)}</div> : null}{adjustment !== null && move.kind === "place" && adjustment !== 0 ? <p className="mt-4 text-sm text-amber-200">Recorded cumulative score includes adjustments beyond this move&apos;s {move.points} points.</p> : null}</section>;
}
