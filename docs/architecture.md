...
- **Kill switch**: Django Admin `is_active` remains catalog authority. Deactivating a row (including NIM) removes it from Settings and fallback queues. Seed and sync never reactivate or deactivate existing rows. Activation and `sort_order` change only through the reviewed admin workflow; ordinary changeform and changelist edits cannot toggle them. The workflow refuses a catalog with zero selectable models under either `DYNAMIC_FREE_MODEL_CATALOG_ENABLED` state and refuses deactivating the last active language model tagged `tools`.
- **Admin capability probes**: Django Admin can record bounded `CapabilityProbe` history on an `AIModel`. The default action is fake (simulated PASS, zero egress). Live probes require `PROVIDER_PROBE_LIVE=1` and a dedicated Node worker with a 20s capability deadline, 22s watchdog, 25s subprocess limit, at most four HTTP dispatches, and a 60s global admission throttle. Probe results never change activation. Django Admin templates escape model IDs, providers, and summaries; they do not use `|safe` or `mark_safe`.
