...
- **Diagnostic targets (S7, FAKE MODE ONLY)**: staff may register a diagnostic-only OpenAI-compatible HTTPS endpoint in Django admin — `DiagnosticAllowedHost` hostname allowlist (eight shipped hosts seeded by migration `0012_diagnostic_targets`; hostnames immutable; add-host does no DNS/HTTP) plus `DiagnosticTarget` (`base_url`, FK allowed host, `model_id`, closed `credential_env_name` choices; connection settings freeze once a `PlayerSlot.diagnostic_target` references the row). One URL policy lives in `backend/game/diagnostic_targets.py` and `frontend/src/lib/diagnostic-target-fetch.ts` (HTTPS, implicit/`:443` only, exact canonical-host equality, no userinfo/IP literals/punycode/trailing dots/queries/fragments/dot segments/percent-escapes; shared negative vectors in `backend/tests/fixtures/diagnostic_ssrf_cases.json`). Save-time DNS is bounded `getaddrinfo` (any private/special address refuses the name); request time uses the bound `node:https` adapter in `diagnostic-target-fetch.ts` (validated IP via custom lookup, dedicated non-reusing agent, no redirects, 1 MiB request / 2 MiB response caps, identity encoding only, fresh all-address DNS per request). Target seats keep `ai_model` null; `get_ai_context` attaches `diagnostic_runtime` only for the diagnostic service user on an active target+host; the ONE SSE route asserts `diagnostic_target_id` against the backend-authorized target, rejects body URL/env/runtime config, and builds the sibling `getDiagnosticLanguageRuntime` (player `getLanguageRuntime` byte-behaviour unchanged). The runner forwards ONLY the target id over worker JSONL. Provider calls stay ZERO in this slice: the worker remains `{mode:"fake"}` and `_worker_env_whitelist` is unchanged.
...
- UI localization now covers gameplay. Twelve variants ship playable — `ls backend/assets/variants/` is 12 and `game.views.list_variant_summaries()` reports `readiness: "playable"` for all twelve — and all twelve have an interface locale: `LOCALES` in `frontend/src/lib/i18n/locales.ts` is `en sk cs pl de pt is it nl da sv af`, `translate.ts` wires all twelve `messages.XX.ts` catalogs, and the interface-language picker offers all twelve endonyms. Variant *names* were already localized: `GameLanguagePanel.tsx` has a `VARIANT_NAME_KEYS` entry for all twelve slugs, so the variant picker and the human-queue label show a translated exonym in every shipped locale, and `lexiconRejectionKey()` names all twelve lexicons on a rejected word. What is still true: the eight newest catalogs (German, Portuguese, Icelandic, Italian, Dutch, Danish, Swedish, Afrikaans) are machine-authored and have had no second-opinion review, and the test suite pins exact expected wording for four of the twelve — `REVIEWED_LOCALES` = `en sk cs pl` — covering the other eight structurally instead. All twelve locales now have a 48x32 flag under `frontend/public/` — the Cooperator supplied the eight new ones himself — and both `LOCALE_FLAG_SRC` in `settings/page.tsx` and `VARIANT_FLAG_SRC` in `GameLanguagePanel.tsx` carry all twelve. ⛔ Both tables stay PARTIAL with a conditional `flagSrc` spread on purpose: an unconditional `/${value}.png` is the defect that shipped eight 404s, and a thirteenth locale must not silently reintroduce it. Second-opinion review of the eight catalogs is open work.
